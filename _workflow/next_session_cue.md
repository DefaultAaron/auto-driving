...
7. Then run Phase-5 per-section deal-loop on §5.10 alone — **Path A (cc-drafted, bidirectional with codex-collaborator every round)**. Re-dispatch cc-writer for ALL revisions (including single-sentence factual / wording fixes); the `main-direct: minor / adjudication` audit tags are deprecated. Only `main-direct: writer-overhead` (narrow: spelling / duplicated word / broken Markdown / format artifact — no semantic change) is available as the rare escape hatch. CONTESTED: pushback applies if codex-collaborator's critique is wrong / off-target / out of scope.
