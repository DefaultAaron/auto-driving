...
[[5_1_pointcloud_preprocessing_EN|§5.1]] establishes five canonical representations (raw point cloud, voxel grid, OctoMap, range image, BEV). §5.2 receives the §5.1 preprocessed raw point cloud — already voxel-downsampled, de-noised, and deskewed — and internally projects it onto whichever representation each ground-segmentation method needs: raw point cloud for RANSAC's KD-tree plane fitting, range image for Himmelsbach scan-line, a radial / polar grid (sectors × distance bins, a voxel grid in cylindrical coordinates rather than the §5.1 flat-Cartesian voxel-grid row) for radial-bin methods and Patchwork's CZM, and BEV grid for grid heightmaps. OctoMap is §5.7-specific and not part of the ground-segmentation pipeline. This section consumes the §5.1 output: a deskewed cloud in `base_link`, voxel-downsampled by `pcl::VoxelGrid`, and de-noised by SOR/ROR. Intensity is not a stable material classifier, so ground segmentation here is geometric; intensity may be passed through for diagnostics, but it does not define asphalt, concrete, water, or vegetation. The optional branches are representation choices: Himmelsbach-style scan-line segmentation uses a range image or ring ordering, while Patchwork and Patchwork++ use a radial-grid view of the same preprocessed cloud.
...
> |---|---|---|---|---|---|
> | `5_2.fm.curb_eaten_as_ground` | Thresholds are loose enough that curb faces and tops join the ground class. | Curbs disappear from the residual cloud; boundary clusters flicker or vanish. | [[5_3_clustering_EN\|§5.3]] misses low road-edge obstacles; [[5_7_occupancy_freespace_map_roi_EN\|§5.7]] may over-clear across a curb. | Use curb-focused validation, range-aware thresholds, smaller near-field Patchwork++ regions, and conservative residual preservation near height discontinuities. | Replay curb approaches, driveway entries, and parking-lot islands; require curb points above the catalog height threshold to remain residual. |
