...
Rain, fog, low light, and sensor degradation all degrade the classical pipeline the same way: noisier sparser returns, intensity that drifts as a signal, and per-stage thresholds tuned on clear weather that start producing rain-spray ghosts and fog-shortened range. The chapter has handled the per-stage half — §5.1's de-noising and intensity caveats, §5.2's ground-segmentation failure modes, §5.3's clustering breakdowns under sparse far returns. The deployment half is operational: validation sets must include adverse-weather rosbags, freshness monitors must detect sensor health degrading faster than the planner can react, and the Generic Obstacle Detection fallback on the ungated occupancy grid must stay active so a DL primary detector that fails on weather-distorted classes still has a class-agnostic backstop. Formal hazard analysis — fault trees, risk acceptance, residual-risk argument — belongs to [[11_0_overview_EN|Ch 11]]; this section catalogs the runtime symptoms.
