...
1. **Preprocessing** — deskew, outlier removal, voxel downsampling, ROI gating. The DL detector still consumes a deskewed, denoised, downsampled cloud; the math from [[5_1_pointcloud_preprocessing_EN|Ch 5 §5.1]] is typically not re-learned end-to-end in shipping stacks.
2. **Ground segmentation as a CNN front-end** — running Patchwork or a Himmelsbach-style radial segmenter before the learned detector reduces the input cloud by 40–60 %. [[5_2_ground_segmentation_EN|Ch 5 §5.2]] is often the classical version in front of an end-to-end-trainable network, since it cuts input cloud size meaningfully before the learned detector runs.
