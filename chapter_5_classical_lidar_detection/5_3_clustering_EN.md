...
Compared with Euclidean clustering, DBSCAN gives a cleaner noise model. Spray returns, glass reflections, or isolated high returns do not need to become singleton clusters if they fail the density test. DBSCAN also handles non-convex shapes because density reachability can follow a curved surface or partial vehicle outline.
...
Bogoslavskyi & Stachniss proposed a fast 3-D LiDAR segmentation method in 2016 that labels connected components directly on this projection. The key test asks whether neighboring pixels lie on the same surface or on different surfaces separated by a depth jump. Let `d1` and `d2` be the shorter and longer ranges of two adjacent pixels, and let `α` be the angular resolution between those beams. If the estimated surface angle is above a threshold, the neighbor is connected; otherwise the depth jump starts a new component.

If two adjacent beams hit the same slanted car side, their ranges differ smoothly and the surface angle is plausible. If one beam hits a pedestrian at 12 m and the next goes to a wall at 30 m, the range discontinuity is too large for one local surface. The algorithm is range-aware because it reasons in angular adjacency rather than fixed metric radius.
...
Deep-learning semantic and instance segmentation displaces much of this in production systems, especially when Ch 6-style networks predict masks or boxes directly. Classical clustering still survives as a fallback, a Generic Obstacle Detection path, a pre-filter for shape fitting, and a safety diagnostic when learned perception is degraded. [[5_9_deployment_runtime_EN|Ch 5 §5.9]] picks up that deployment role.
