...
where `T_base_world(t) = (T_world_base(t))^-1`. `T_base_lidar` is the calibrated extrinsic from [[1_1_coordinate_frames_EN|Ch 1 §1.1]] / [[1_3_lidar_calibration_EN|Ch 1 §1.3]]. The chapter convention (committed in [[5_8_ros2_integration_EN|§5.8]]) is `header.stamp` = end-of-scan reference time + per-point `time` field as a non-negative `Δt`, so `t_point = header.stamp - point.time`. Drivers that publish offset-from-sweep-start, absolute GPS time, or omit the field must be normalized at the message boundary — the §5.1 deskew code consumes the chapter convention, not whatever the driver happened to emit. `t_common` is commonly the sweep-end time because it matches the latest ego pose available at publication, but midpoint deskew is also valid if every downstream consumer agrees. The persisted frame is usually `base_link` at `t_common`; `odom` or `map` may be used for registration and ROI branches, but the invariant is still one common time.
...
### 5.1.7 Output contract for downstream sections
