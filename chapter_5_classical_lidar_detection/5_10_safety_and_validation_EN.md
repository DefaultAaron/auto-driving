...
These four entries each illustrate a different gap: severity, topology, track continuity, behaviour. The missing half is **scenario-based testing** — replay curated logs with engineering-grade ground truth (free-space polygons, track continuity, gap correctness) and assert behaviour, not per-frame overlap. [[11_3_scenario_based_testing_EN|Ch 11 §11.3]] owns the methodology. mAP is necessary, not sufficient.
