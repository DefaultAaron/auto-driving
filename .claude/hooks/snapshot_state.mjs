...
//
// Does NOT touch reasoning fields (active_batch, last_agreed_commit,
// next_action, open_conflict_threads, blocked_user_inputs, do_not_redo).
// Those are owned by main session.
//
...
const STATE_PATH = resolve(VAULT_ROOT, '_workflow/STATE.md');
const WORKTREE_PATH = resolve(VAULT_ROOT, '..', 'auto-driving-codex-worktree');
const SENTINEL_PATH = resolve(VAULT_ROOT, '.claude/active_writer_batch.json');

if (!existsSync(STATE_PATH)) {
  // No STATE.md yet — no-op. Vaults that haven't adopted STATE.md still get clean compaction.
  process.exit(0);
}
...
const head = gitOutput(['rev-parse', 'HEAD'], VAULT_ROOT);
const mainStatus = gitOutput(['status', '--porcelain'], VAULT_ROOT);
