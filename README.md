...
|---|---|---|---|
| Main session | Claude Code (this CLI) | yes — sole authority for `CLAUDE.md`, `README.md`, memory, TOC, chapter overviews, final commits | orchestrator + **conflictor for codex-drafted sections (Phase 5 Path B rounds 1..N-1)** |
| `codex-collaborator` | codex-companion runtime, no `--write` | no | RESEARCH \| CONFLICT (cc-drafted every round / codex-drafted final-round sanity pass only) |
| `gemini-researcher` | Gemini CLI, `--approval-mode plan` | no | RESEARCH only + content-risk-triggered factual spot-check |
| `cc-writer` | Claude Code subagent | yes — only the batch-assigned section path, hook-enforced | WRITER only (drafts AND revisions) |
| `codex-writer` | codex-companion runtime, `--write`, `--cwd` to sacrificial worktree | only the assigned section path is copied back to main repo | WRITER only (drafts AND revisions) |
...
5. **Per-section deal-loop** — **asymmetric by writer model** (post-2026-05-02):
   - **Path A — cc-drafted sections.** Main + codex-collaborator CONFLICT iterate every round under bidirectional convergence; both must AGREE; main may push back via `CONTESTED:`.
   - **Path B — codex-drafted sections.** Main session conflicts rounds 1..N-1 unilaterally (with the four named bias axes — markdown over-listing, analogy register, foundational example choice, depth defaults — folded into main's prompt as Rule 3c absorption). Before declaring AGREED, main dispatches codex-collaborator for one final-round sanity pass at round N. Any fix to a codex-collaborator final-round critique requires another codex-collaborator pass on the changed text — convergence requires codex-collaborator AGREED on the actual final commit.
   - For codex-drafted sections, main also dispatches `gemini-researcher` for a factual spot-check on 2–3 claims (Rule 3b — content-risk-triggered, rerun on factual changes only).
   - **All section-content writing — drafts AND revisions — goes through writers.** The `main-direct: minor` and `main-direct: adjudication` audit tags are deprecated; only `main-direct: writer-overhead` survives with a narrow definition (spelling, duplicated word, broken Markdown, obvious syntax/format artifact — no semantic changes, no sentence rewrites).
...
**Default ratio under round-W (user billing constraint, 2026-05-03)**: **all-codex-writer** for both initial drafts and extension dispatches. cc-writer reserved as a Rule 3d round-4+ user-approval-gated escape hatch only. The Phase-3 chapter-plan classification (contested-framing / theoretically-loaded / contested-combination / novel-section-framing / synthesis) is **still recorded** because it sets the protected-framing-spans rationale that Safeguard 1 below depends on; the writer-allocation default is codex-with-five-safeguards rather than cc-writer.

**Original ratio (pre-round-W; historical record)**: codex-writer-default with cc-writer reserved for chapter-classified contested-framing / high-judgment-synthesis / novel-pedagogical-integration sections. Expected ~2cc : 8codex per chapter. Original Ch 5 used 5cc:5codex (after §5.10 reassignment).

**Five binding safeguards under round-W** (required because the cc-writer rationale is being overridden, not erased):
1. **Protected framing spans** declared verbatim in every brief for sections originally classified contested-framing / high-judgment-synthesis. codex-writer extends *around* these spans, does not paraphrase them.
2. **Synthesis-section mandatory terminal consistency audit** (row-by-row diff against a change log; vocabulary normalization scan; duplicate-hazard scan; whole-table sanity pass) as round-1 main-conflict critique input AND final-round-sanity-pass-blocking.
...
4. **`framing-preservation` Path B axis** added to the four Rule 3c axes (markdown over-listing / analogy register / foundational example / depth defaults). Checks: protected thesis unchanged, scoped claims unchanged, no broadened production claims, no renamed contract terms.
5. **Section-specific deterministic rules retired** (the §5.4-class "all-cc-by-default-or-split-on-isolation" rule is superseded by all-codex-with-protected-framing).

See `feedback_billing_constraint_writer_allocation.md` for the full rule, and `_workflow/plans/ch5_extension_plan.md` §3.2 for the Ch 5 extension safeguard description.

**Bias mitigation — primarily structural via Phase 5 Path A/B asymmetry, plus residual procedural rules:**
- **Rule 3a — codex-default allocation; under round-W, cc-writer downgraded to Rule 3d escape hatch only.** The Phase-3 classification still happens (it sets the protected-framing-spans rationale); the writer-allocation default is codex-with-safeguards.
- **Rule 3b — gemini factual spot-check on codex-drafted sections, content-risk-triggered.** Main selects 2–3 claims, dispatches gemini, proceeds toward final-round codex-collaborator AGREED only after verification. Rerun when round-1 commits substantive factual claims OR a subsequent revision adds/materially changes factual claims; do NOT auto-rerun per round.
- **Rule 3c — codex-bias checklist axes folded into main's Path B conflict-review prompt.** As of post-2026-05-02, Rule 3c as a separate procedural step is deprecated; the four axes — **markdown over-listing**, **analogy register**, **foundational example choice**, **depth defaults** — are part of every Phase-5 Path B round's main-conflict review. **Under round-W: a fifth axis `framing-preservation` is added for flipped former-cc sections.** Main's critique must enumerate which of the five axes (if any) flagged.
- **Rule 3d — round-4+ writer-side break.** When main has raised the same critique 3+ times on a codex-drafted section without convergence, main *recommends* a cc-writer fresh-eye revision on the disputed passage. **Under round-W: mandatory user-approval gate before any cc-writer dispatch.** Default fallback if user declines: main-direct revision (codex-collaborator must AGREE) or another narrowed-brief codex-writer round.
...
2. Add the section to the Phase 3 chapter plan (scope, depth, length band, dependency edges, writer assignment per Rule 3a — codex-default unless the section is chapter-classified as contested-framing / high-judgment-synthesis / novel-pedagogical-integration — handoff snippet if dependent).
